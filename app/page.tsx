...
const MagicCard = ({ title, attraction, desc, delay, onClick, badge, children }: any) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="magic-card" style={{ transform: `perspective(1200px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg) scale3d(1, 1, 1)` }}>
        <div className="card-glare" style={{ transform: `translate(${tilt.y * 4}px, ${-tilt.x * 4}px)` }} />
        {badge && <span className="badge-new">{badge}</span>}
        
        {/* ✨ 中身が浮き出る（translateZ）視差効果 */}
...
export default function ThemeParkEntrance() {
  ...
  const [newsText, setNewsText] = useState("【Welcome】本日はご来園ありがとうございます！魔法のワークスペースへようこそ！");
  ...
  const [utilResult, setUtilResult] = useState("魔法の杖で探したい住所を入力してください");
  const [isSearching, setIsSearching] = useState(false);

  useEffect(() => {
    const zipCode = utilInput.replace(/[^0-9]/g, "");
    if (zipCode.length === 7) {
      setIsSearching(true); setUtilResult("✨ 魔法陣を展開中...");
      fetch(`https://zipcloud.ibsnet.co.jp/api/search?zipcode=${zipCode}`)
        ...
        }).catch(() => setUtilResult("⚠️ 通信に乱れが生じています")).finally(() => setIsSearching(false));
    } else if (utilInput.length > 0 && !utilInput.match(/[0-9]/)) setUtilResult(`🔤 カナ変換魔法は現在習得中です`);
    else if (utilInput.length === 0) setUtilResult("魔法の杖で探したい住所を入力してください");
  ...
  const handleClockOut = async () => {
    ...
    try { await navigator.clipboard.writeText(`${hours}:${minutes} 退勤いたします`); showToast(`✨ 退勤の魔法をコピーしました！またお越しください！`); } catch (err) { alert("失敗しました"); }
  };
  ...
  const copyUtilResult = async () => { if (utilResult.includes("🏰")) { try { await navigator.clipboard.writeText(utilResult.replace("🏰 ", "")); showToast("📋 住所をクリップボードに転送しました！"); } catch (err) { alert("失敗しました"); } } };
    ...
    <>
      ...
      <main className={`app-wrapper ${isReady ? "ready" : ""}`}>
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          /* 🌟 高度技術④：SVGアニメーション（背景に浮かぶシンボルキャッスル） */
          .magic-svg-bg { position: fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 100vw; max-width: 1200px; height: auto; z-index: -1; pointer-events: none; opacity: 0.15; }
          .castle-path { fill: none; stroke: url(#goldGradient); stroke-width: 2; stroke-dasharray: 4000; stroke-dashoffset: 4000; animation: drawCastle 15s ease-in-out forwards; filter: drop-shadow(0 0 15px rgba(250,204,21,0.6)); }
          @keyframes drawCastle { to { stroke-dashoffset: 0; } }
          ...
          .btn-clockout:hover { transform: translateY(-3px); box-shadow: 0 15px 35px rgba(225, 29, 72, 0.6); }
          ...
          .btn-sim:hover { background: rgba(30, 41, 59, 0.9); transform: translateY(-3px); border-color: #fde047; box-shadow: 0 15px 35px rgba(250, 204, 21, 0.3); }
          ...
          /* 🌟 高度技術⑤＆⑥：アトラクションカード（SASSライクなディレイと具現化） */
          .attraction-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 35px; perspective: 1500px; }
          
          .fade-up-element { opacity: 0; transform: translateY(80px) scale(0.9); transition: all 1s cubic-bezier(0.2, 0.8, 0.2, 1); transition-delay: var(--delay); }
          .fade-up-element.visible { opacity: 1; transform: translateY(0) scale(1); }

          .magic-card { background: rgba(15, 23, 42, 0.7); backdrop-filter: blur(25px); border: 1px solid rgba(255, 255, 255, 0.1); border-top: 1px solid rgba(255,255,255,0.2); border-radius: 28px; padding: 35px 30px; cursor: pointer; position: relative; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.8); display: flex; flex-direction: column; gap: 15px; transform-style: preserve-3d; transition: border-color 0.4s; }
          .magic-card-wrapper:hover .magic-card { border-color: rgba(250, 204, 21, 0.6); }
          ...
          /* ✨ 中身の3D浮き出し (Z軸パララックス) */
          .card-content-3d { z-index: 2; position: relative; transform: translateZ(50px); transform-style: preserve-3d; transition: transform 0.2s ease-out; }
          ...
          .badge-new { position: absolute; top: 20px; right: 20px; background: linear-gradient(135deg, #ef4444, #b91c1c); color: #fff; font-size: 11px; font-weight: 900; padding: 6px 14px; border-radius: 20px; z-index: 3; box-shadow: 0 5px 15px rgba(225,29,72,0.5); letter-spacing: 2px; }

          /* KPI内部要素 */
          .kpi-widget { margin-top: 15px; background: rgba(0,0,0,0.4); padding: 15px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05); transform: translateZ(30px); }
        ...
        {/* 🌟 高度技術④：SVGアニメーション背景（シンボルキャッスル） */}
        <svg className="magic-svg-bg" viewBox="0 0 800 600" preserveAspectRatio="xMidYMax slice">
          <defs>
          ...
          <path className="castle-path" d="M 400,100 L 400,200 L 450,200 L 450,300 L 500,300 L 500,500 L 300,500 L 300,300 L 350,300 L 350,200 L 400,200 M 400,100 L 380,150 L 420,150 Z M 450,200 L 430,250 L 470,250 Z M 350,200 L 330,250 L 370,250 Z" />
        </svg>

        <div className="dashboard-inner">
          <div className="top-bar">
            <button className="theme-toggle-btn" onClick={toggleTheme}>{isDarkMode ? "🎇" : "🌅"}</button>
            <div className="user-profile">
              <div className="avatar-circle">{userName.charAt(0).toUpperCase()}</div>
              <div className="greeting-text"><span>Guest ID</span><span className="user-id-text">{userName}</span></div>
            </div>
          </div>
          ...
          <div className="park-title-container fade-up-element" style={{ "--delay": "0s" } as any}>
            ...
            <div className="park-sub-title">Magical Operations Center</div>
            <div className="quick-actions">
              <button className="btn-qa btn-sim" onClick={handleOpenSim}>⏳ 納期確認 (ファストパス)</button>
              <button className="btn-qa btn-clockout" onClick={handleClockOut}>🏃‍♂️ パークを退園する</button>
            </div>
          </div>

          <div className="news-ticker-wrapper fade-up-element" style={{ "--delay": "0.1s" } as any}>
            <div className="news-badge">📢 パーク・インフォメーション</div>
          ...
          <div className="attraction-grid">
            <MagicCard delay={0.1} attraction="TOY STORY MANIA!" title="🎯 獲得進捗・KPI" desc="おもちゃの世界でスコア（件数）を稼ごう！チームの獲得状況やランキングを確認できます。" onClick={() => setIsKpiOpen(true)}>
              <div className="kpi-widget">
                <div className="kpi-numbers"><span className="kpi-current">{mockKpi.current}</span><span className="kpi-target">/ 目標 {mockKpi.target}件</span></div>
                <div className="kpi-bar-bg"><div className="kpi-bar-fill" style={{ width: `${progressPercent}%` }}></div></div>
              </div>
            </MagicCard>

            <MagicCard delay={0.2} attraction="PIRATES OF THE CARIBBEAN" title="💰 料金シミュレーター" badge="NEW" desc="隠された財宝（節約額）を発見せよ！お客様の利用状況から乗り換え時のトク額を即座に計算します。" onClick={() => router.push("/simulator")} />
            
            <MagicCard delay={0.3} attraction="SOARING: FANTASTIC FLIGHT" title="🗺️ Kraken 魔法の地図" badge="NEW" desc="大空を巡るように、プラン変更やキャンセル確認など、Krakenの正しい操作手順を優雅に導きます。" onClick={() => router.push("/procedure-wizard")} />
            
            <MagicCard delay={0.4} attraction="TOWER OF TERROR" title="⚡ トラブル解決ナビ" badge="NEW" desc="恐怖のエラーから生還せよ！SWエラーやメアド重複などのイレギュラー対応を安全にナビゲートします。" onClick={() => router.push("/trouble-nav")} />
            
            <MagicCard delay={0.5} attraction="SPACE MOUNTAIN" title="🚀 データ一括登録" desc="宇宙空間を光の速さでワープ！複数の顧客データをデータベースへ高速転送します。" onClick={() => router.push("/bulk-register")} />
            
            <MagicCard delay={0.6} attraction="JOURNEY TO THE CENTER" title="🌋 自己クロ連携ツール" desc="地底深くのコアへ潜入！成約後の確実な情報をシームレスかつスピーディーに連携します。" onClick={() => router.push("/self-close")} />
            
            <MagicCard delay={0.7} attraction="SPLASH MOUNTAIN" title="🌊 ネットトス連携ツール" desc="滝壺への大ダイブ！ネット回線のトスアップデータを指定のシートへ一気に流し込みます。" onClick={() => router.push("/net-toss")} />
            
            <MagicCard delay={0.8} attraction="MONSTERS, INC." title="🔦 SMS (Kraken) 送信" desc="フラッシュライトで的確に狙え！お客様へSMSを送信・履歴管理・テンプレート展開を行います。" onClick={() => router.push("/sms-kraken")} />
            
            <MagicCard delay={0.9} attraction="IT'S A SMALL WORLD" title="🎠 メールテンプレート" desc="世界中どこでも通じる魔法の言葉。用途に応じた美しいメール文面を素早く作成します。" onClick={() => router.push("/email-template")} />
            
            <MagicCard delay={1.0} attraction="POOH'S HUNNY HUNT" title="🍯 クイック一時メモ" desc="はちみつのように甘くて便利な、一時保存用の魔法のスクラッチパッド。" onClick={() => setIsMemoOpen(true)}>
              {memoText && <div style={{padding:"12px", background:"rgba(0,0,0,0.4)", borderRadius:"8px", fontSize:"13px", color:"#fde047", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", border:"1px dashed rgba(250,204,21,0.3)", transform:"translateZ(30px)"}}>{memoText}</div>}
            </MagicCard>
          </div>
        </div>
