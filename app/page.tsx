    ...
    <>
      ...
      <main className={`app-wrapper ${isReady ? "ready" : ""}`}>
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .park-main-title { 
            font-size: 60px; font-weight: 900; letter-spacing: 3px; margin: 0 0 15px 0;
            background: linear-gradient(to right, #fde047, #fbcfe8, #38bdf8, #fde047);
            background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent;
            animation: shine 4s linear infinite; clip-path: polygon(0 0, 100% 0, 100% 100%, 0 100%);
            filter: drop-shadow(0 4px 10px rgba(253,224,71,0.3));
          }
          ...
          .fade-up-element { opacity: 0; transform: translateY(60px); transition: opacity 0.8s cubic-bezier(0.2, 0.8, 0.2, 1), transform 0.8s cubic-bezier(0.2, 0.8, 0.2, 1); transition-delay: var(--delay); }
          .fade-up-element.visible { opacity: 1; transform: translateY(0); }

          .magic-card { background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.15); border-radius: 24px; padding: 30px 25px; cursor: pointer; position: relative; overflow: hidden; transition: transform 0.1s ease-out, box-shadow 0.2s ease-out, border-color 0.2s; box-shadow: 0 15px 35px rgba(0,0,0,0.6); display: flex; flex-direction: column; gap: 12px; transform-style: preserve-3d; }
          ...
          /* ✨ 3Dで中身が浮き出る */
          .card-content-3d { z-index: 2; position: relative; transform: translateZ(40px); transform-style: preserve-3d; }
          
          .card-title { font-size: 18px; font-weight: 900; color: #fff; margin: 0; text-shadow: 0 2px 5px rgba(0,0,0,0.9); line-height: 1.4; letter-spacing: 0.5px; display: flex; align-items: center; gap: 8px; }
          .card-desc { font-size: 13px; color: #cbd5e1; margin: 0; line-height: 1.6; font-weight: 700; text-shadow: 0 1px 3px rgba(0,0,0,0.9); transform: translateZ(20px); }
          .card-custom-inner { margin-top: 10px; transform: translateZ(30px); }
          ...
          /* 内部要素（KPI・メモ） */
          .kpi-widget { background: rgba(0,0,0,0.4); padding: 12px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); }
          ...
          .memo-preview { padding: 8px 12px; background: rgba(0,0,0,0.4); border-radius: 8px; font-size: 12px; color: #cbd5e1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; border: 1px dashed rgba(255,255,255,0.2); }
          ...
          .utility-content { position: absolute; bottom: 90px; right: 0; width: 340px; background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(25px); padding: 25px; border-radius: 24px; box-shadow: 0 25px 60px rgba(0,0,0,0.8); border: 1px solid rgba(255,255,255,0.2); color: #fff; }
