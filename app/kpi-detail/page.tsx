...
export default function KpiDetailDashboard() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(true); // 分析系はダークがかっこいいので初期値True
    ...
    const fetchWithJSONP = () => {
      ...
      (window as any)[callbackName] = (jsonData: any) => {
        if(jsonData.success) { 
          ...
          // 💡 【超重要】HOME画面に反映させるためにローカルストレージへ保存！！
  ...
  if (errorMsg) {
    return (
      <div style={{ background: "var(--bg-gradient)", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", padding: "40px", textAlign: "center", color: "var(--text-main)", fontFamily: "'Inter', sans-serif" }}>
        ...
        <button onClick={() => router.push("/")} style={{padding: "12px 30px", marginTop: "20px", background: "linear-gradient(135deg, #0ea5e9, #0284c7)", color: "#fff", border: "none", borderRadius: "12px", fontWeight: 900, cursor: "pointer"}}>◀ HOMEへ戻る</button>
      </div>
    );
  }
      ...
      <main className={`app-wrapper ${isDarkMode ? "theme-dark" : "theme-light"} ${isReady ? "ready" : ""}`}>
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .theme-light { 
            ...
            --table-header: rgba(14, 165, 233, 0.1);
          ...
          .side-link.current-page { background: linear-gradient(135deg, var(--nav-accent), #0284c7); color: #fff; border: none; pointer-events: none; }
          ...
          .mode-btn.active { background: var(--nav-accent); color: #fff; box-shadow: 0 4px 15px rgba(14, 165, 233, 0.4); }
          ...
          /* 進捗リング */
          .ring-container { display: flex; flex-direction: column; align-items: center; justify-content: center; margin-bottom: 20px; }
          .ring-svg { width: 200px; height: 200px; transform: rotate(-90deg); }
          .ring-bg { fill: none; stroke: var(--input-bg); stroke-width: 4; }
          .ring-fill { fill: none; stroke: var(--nav-accent); stroke-width: 4; stroke-linecap: round; stroke-dasharray: 100; stroke-dashoffset: ${100 - progressPercent}; transition: 1.5s cubic-bezier(0.2, 0.8, 0.2, 1); }
          
          .ring-text-container { position: absolute; display: flex; flex-direction: column; align-items: center; justify-content: center; }
          .ring-percent { font-size: 36px; font-weight: 900; color: var(--text-main); line-height: 1; }
          .ring-target { font-size: 14px; font-weight: 800; color: var(--text-sub); margin-top: 4px; }
          ...
          .op-prog-fill { height: 100%; background: linear-gradient(90deg, #0ea5e9, #8b5cf6); transition: width 1.5s cubic-bezier(0.2, 0.8, 0.2, 1); border-radius: 4px; }
          ...
          .cell-highlight { color: var(--nav-accent) !important; font-weight: 900 !important; background: rgba(14, 165, 233, 0.05); }
          .theme-dark .cell-highlight { color: #fde047 !important; background: rgba(253, 224, 71, 0.05); }
