...
export default function SimulatorTest() {
  ...
  useEffect(() => {
    if (!FREE_HALF_DATA[freeLine][freePlan]) setFreePlan("MS");
  }, [freeLine]);
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('.fade-up-element').forEach((el) => {
      el.classList.remove("visible"); // タブ切り替え時に再アニメーション
      observer.observe(el);
    });
  ...
  return (
    <>
      {/* 🌟 Dynamic Theming Container 🌟 */}
      <div data-theme={activeTab} className={`app-container ${isDarkMode ? "theme-dark" : "theme-light"}`}>
        ...
        <div className="entrance-bg">
          ...
          <svg className="data-svg-bg" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <path className="data-path" d="M -10,80 L 40,80 L 60,30 L 90,30 L 110,80" style={{animationDelay: "3s", opacity: 0.3}} />
          </svg>
        </div>
        ...
        <main className={`app-wrapper ${isReady ? "ready" : ""}`}>
          <style dangerouslySetInnerHTML={{ __html: `
            ...
            /* 🎨 Dynamic Theme Generator (Generative UI) */
            [data-theme="sb-hikari"] { --accent-rgb: 14, 165, 233; --bg-grad-light: linear-gradient(135deg, #f0f9ff, #e0f2fe, #bae6fd); --bg-grad-dark: radial-gradient(ellipse at top left, #082f49, #020617); } /* Cyan */
            [data-theme="sb-air"] { --accent-rgb: 249, 115, 22; --bg-grad-light: linear-gradient(135deg, #fff7ed, #ffedd5, #fed7aa); --bg-grad-dark: radial-gradient(ellipse at top left, #431407, #020617); } /* Orange */
            [data-theme="docomo-hikari"] { --accent-rgb: 225, 29, 72; --bg-grad-light: linear-gradient(135deg, #fff1f2, #ffe4e6, #fecdd3); --bg-grad-dark: radial-gradient(ellipse at top left, #4c0519, #020617); } /* Rose */
            [data-theme="flets-hikari"] { --accent-rgb: 79, 70, 229; --bg-grad-light: linear-gradient(135deg, #eef2ff, #e0e7ff, #c7d2fe); --bg-grad-dark: radial-gradient(ellipse at top left, #1e1b4b, #020617); } /* Indigo */
            [data-theme="free-half"] { --accent-rgb: 16, 185, 129; --bg-grad-light: linear-gradient(135deg, #ecfdf5, #d1fae5, #a7f3d0); --bg-grad-dark: radial-gradient(ellipse at top left, #022c22, #020617); } /* Emerald */
            [data-theme="air-talk"] { --accent-rgb: 168, 85, 247; --bg-grad-light: linear-gradient(135deg, #faf5ff, #f3e8ff, #e9d5ff); --bg-grad-dark: radial-gradient(ellipse at top left, #3b0764, #020617); } /* Purple */

            /* Calm Design & Accessibility Modes */
            .theme-light { --text-main: #1e293b; --text-sub: #475569; --card-bg: rgba(255, 255, 255, 0.7); --card-border: rgba(255, 255, 255, 0.6); --card-shadow: 0 8px 32px rgba(0,0,0,0.04); --input-bg: rgba(255,255,255,0.8); }
            .theme-dark { --text-main: #f8fafc; --text-sub: #94a3b8; --card-bg: rgba(15, 23, 42, 0.65); --card-border: rgba(255, 255, 255, 0.08); --card-shadow: 0 16px 40px rgba(0,0,0,0.6); --input-bg: rgba(0,0,0,0.4); }
            
            .entrance-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: -2; transition: background 0.8s ease; }
            .theme-light .entrance-bg { background: var(--bg-grad-light); }
            .theme-dark .entrance-bg { background: var(--bg-grad-dark); }

            /* Immersive SVG & Particles */
            .particles-container { position: absolute; top: 0; left: 0; width: 100%; height: 100%; overflow: hidden; pointer-events: none; }
            .particle { position: absolute; border-radius: 50%; background: rgb(var(--accent-rgb)); box-shadow: 0 0 12px rgb(var(--accent-rgb)); opacity: 0.3; animation: flowUp 10s infinite ease-in-out; transition: background 0.5s, box-shadow 0.5s; }
            @keyframes flowUp { 0% { opacity: 0; transform: translateY(20px) scale(0.5); } 50% { opacity: 0.5; transform: translateY(-50px) scale(1.2); } 100% { opacity: 0; transform: translateY(-100px) scale(0.5); } }
            .data-svg-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: -1; pointer-events: none; opacity: 0.6; }
            .data-path { fill: none; stroke: rgba(var(--accent-rgb), 0.3); stroke-width: 2; stroke-dasharray: 1500; stroke-dashoffset: 1500; animation: drawDataFlow 20s linear infinite; transition: stroke 0.5s; }
            ...
            .glass-nav { display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; background: var(--card-bg); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid var(--card-border); border-radius: 50px; box-shadow: var(--card-shadow); max-width: 800px; width: 100%; transition: 0.3s;}
            ...
            .glass-nav-active { padding: 10px 20px; border-radius: 30px; font-weight: 900; background: rgba(var(--accent-rgb), 0.1); color: rgb(var(--accent-rgb)); border: 1px solid rgba(var(--accent-rgb), 0.3); font-size: 14px; transition: 0.3s; }
            ...
            .theme-toggle-btn:hover { border-color: rgb(var(--accent-rgb)); color: rgb(var(--accent-rgb)); transform: scale(1.05);}
            
            /* Vertical UI & Sim Tabs */
            .sim-tabs-wrapper { display: flex; justify-content: center; margin-bottom: 30px; perspective: 1000px;}
            .sim-tabs { display: flex; background: var(--input-bg); padding: 6px; border-radius: 20px; border: 1px solid var(--card-border); box-shadow: var(--card-shadow); gap: 5px; flex-wrap: wrap; justify-content: center; backdrop-filter: blur(10px);}
            .sim-tab-btn { padding: 10px 16px; border: none; border-radius: 14px; font-size: 13px; font-weight: 900; cursor: pointer; transition: all 0.4s cubic-bezier(0.2, 0.8, 0.2, 1); background: transparent; color: var(--text-sub); position: relative; overflow: hidden; z-index: 1;}
            .sim-tab-btn::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgb(var(--accent-rgb)); opacity: 0; z-index: -1; transition: 0.4s; }
            .sim-tab-btn.active { color: #fff; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(var(--accent-rgb), 0.3); pointer-events: none; }
            .sim-tab-btn.active::before { opacity: 1; }
            .sim-tab-btn:hover:not(.active) { color: rgb(var(--accent-rgb)); background: rgba(var(--accent-rgb), 0.05); }

            /* Bento UI & 3D Parallax Hover */
            ...
            .glass-panel { background: var(--card-bg); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid var(--card-border); border-radius: 24px; padding: 30px; box-shadow: var(--card-shadow); transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); position: relative; overflow: hidden; display: flex; flex-direction: column; gap: 20px; transform: translateZ(0);}
            .glass-panel:hover { transform: translateY(-4px) translateZ(0); box-shadow: 0 20px 40px rgba(var(--accent-rgb), 0.15); border-color: rgba(var(--accent-rgb), 0.4); }
            .glass-panel.highlight-panel::before { content: ''; position: absolute; top: 0; left: 0; width: 6px; height: 100%; background: rgb(var(--accent-rgb)); opacity: 0.8; transition: background 0.5s;}
            
            /* Dynamic Typography & Masking */
            .section-title { font-weight: 900; font-size: 18px; margin-bottom: 5px; border-bottom: 2px dashed var(--card-border); padding-bottom: 15px; background: linear-gradient(135deg, rgb(var(--accent-rgb)), var(--text-main)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; transition: 0.5s;}
            
            /* Micro-interactions: Sliders & Boxes */
            .slider-container { display: flex; flex-direction: column; gap: 10px; background: var(--input-bg); padding: 20px; border-radius: 16px; border: 1px solid var(--input-border); transition: 0.3s;}
            .slider-container:focus-within { border-color: rgb(var(--accent-rgb)); box-shadow: 0 0 0 3px rgba(var(--accent-rgb), 0.1); }
            ...
            .slider-value { font-size: 32px; font-weight: 900; color: rgb(var(--accent-rgb)); text-shadow: 0 2px 10px rgba(var(--accent-rgb), 0.2); transition: 0.3s;}
            ...
            .util-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 24px; height: 24px; border-radius: 50%; background: rgb(var(--accent-rgb)); cursor: pointer; border: 2px solid #fff; box-shadow: 0 0 10px rgba(0,0,0,0.1); transition: 0.2s;}
            .util-slider::-webkit-slider-thumb:hover { transform: scale(1.2); }
            
            .result-grid { display: grid; grid-template-columns: 1fr; gap: 10px; }
            .result-box { display: flex; justify-content: space-between; align-items: center; background: var(--input-bg); padding: 14px 18px; border-radius: 12px; border: 1px solid var(--input-border); transition: all 0.3s cubic-bezier(0.2, 0.8, 0.2, 1); }
            .result-box:hover { border-color: rgba(var(--accent-rgb), 0.5); transform: translateX(5px); background: rgba(var(--accent-rgb), 0.02); }
            ...
            /* Action-First Toggles */
            .toggle-btn { background: var(--card-bg); border: 1px solid var(--card-border); padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 900; cursor: pointer; transition: all 0.3s; color: var(--text-sub); letter-spacing: 1px;}
            .toggle-btn.on { background: rgba(var(--accent-rgb), 0.15); border-color: rgb(var(--accent-rgb)); color: rgb(var(--accent-rgb)); box-shadow: 0 2px 10px rgba(var(--accent-rgb), 0.2);}
            .toggle-btn.off { background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.3); color: #ef4444; }
            .toggle-btn:hover { transform: scale(1.05); }
            
            .total-box { background: linear-gradient(135deg, rgba(var(--accent-rgb), 0.1), rgba(var(--accent-rgb), 0.05)); border: 2px solid rgb(var(--accent-rgb)); margin-top: 10px;}
            .total-box .result-label { color: rgb(var(--accent-rgb)); font-size: 15px; }
            .total-box .result-value { font-size: 28px; color: rgb(var(--accent-rgb)); text-shadow: 0 2px 5px rgba(var(--accent-rgb), 0.3); }
            
            /* Talk Scripts / Chat Bubbles */
            .script-bubble { background: var(--input-bg); padding: 20px; border-radius: 16px 16px 16px 0; border: 1px solid var(--input-border); font-size: 14px; line-height: 1.8; font-weight: 700; color: var(--text-main); position: relative; margin-bottom: 15px; border-left: 4px solid rgb(var(--accent-rgb)); transition: 0.4s;}
            .script-bubble::after { content: ''; position: absolute; bottom: -10px; left: -1px; border-width: 10px 10px 0 0; border-style: solid; border-color: rgb(var(--accent-rgb)) transparent transparent transparent; transition: 0.4s;}
            .script-bubble:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.05); }
            .script-highlight { color: rgb(var(--accent-rgb)); font-weight: 900; background: rgba(var(--accent-rgb), 0.1); padding: 1px 4px; border-radius: 4px; transition: 0.3s;}
            
            .script-guest { background: rgba(0,0,0,0.03); border-left: 4px solid #64748b; margin-left: 20px; border-radius: 16px 16px 0 16px;}
            .theme-dark .script-guest { background: rgba(255,255,255,0.05); }
            ...
            .script-me { background: rgba(var(--accent-rgb), 0.05); border-left: 4px solid rgb(var(--accent-rgb)); }
            .script-me::after { border-color: rgb(var(--accent-rgb)) transparent transparent transparent; }

            /* 🪄 Scroll Trigger (Advanced Masking/Blur) */
            .fade-up-element { opacity: 0; transform: translateY(30px) scale(0.95); filter: blur(8px); transition: all 0.7s cubic-bezier(0.2, 0.8, 0.2, 1); }
            .fade-up-element.visible { opacity: 1; transform: translateY(0) scale(1); filter: blur(0); }

            /* VS Cards (3D Depth) */
            .vs-container { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 20px; }
            .vs-card { padding: 15px; border-radius: 16px; border: 1px solid var(--card-border); background: var(--input-bg); position: relative; transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1);}
            .vs-card:hover { transform: translateY(-4px) scale(1.02); box-shadow: 0 12px 24px rgba(0,0,0,0.1); z-index: 10;}
            .vs-card.bad { border-color: #ef4444; background: rgba(239, 68, 68, 0.05); }
            .vs-card.good { border-color: rgb(var(--accent-rgb)); background: rgba(var(--accent-rgb), 0.05); border-width: 2px;}
            ...
            .plan-btn { flex: 1; padding: 10px 4px; border: none; border-radius: 8px; font-weight: 900; font-size: 11px; cursor: pointer; background: transparent; color: var(--text-sub); transition: 0.3s; text-align: center; white-space: nowrap;}
            .plan-btn.active { background: rgb(var(--accent-rgb)); color: #fff; box-shadow: 0 4px 12px rgba(var(--accent-rgb), 0.3); transform: translateY(-1px);}
            ...
            .select-input:hover { border-color: rgb(var(--accent-rgb)); box-shadow: 0 0 0 3px rgba(var(--accent-rgb), 0.1); }
            
            .prompter-tabs { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 15px; }
            .prompter-btn { padding: 8px 12px; border-radius: 8px; font-size: 11px; font-weight: 900; border: 1px solid var(--card-border); background: var(--card-bg); color: var(--text-sub); cursor: pointer; transition: 0.3s; }
            .prompter-btn.active { background: rgba(var(--accent-rgb), 0.1); color: rgb(var(--accent-rgb)); border-color: rgb(var(--accent-rgb)); box-shadow: 0 2px 8px rgba(var(--accent-rgb), 0.15); transform: translateY(-1px);}
            .prompter-btn:hover:not(.active) { border-color: rgba(var(--accent-rgb), 0.5); color: rgb(var(--accent-rgb)); }
            ...
            .stylish-toggle-slider { position: absolute; top: 4px; bottom: 4px; width: calc(50% - 4px); background: rgb(var(--accent-rgb)); border-radius: 26px; transition: transform 0.4s cubic-bezier(0.25, 0.8, 0.25, 1); box-shadow: 0 4px 12px rgba(var(--accent-rgb), 0.4); z-index: 1;}
            ...
            #toast { visibility: hidden; position: fixed; bottom: 30px; right: 30px; background: rgba(var(--accent-rgb), 0.9); color: #fff; backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.2); padding: 12px 24px; border-radius: 12px; font-weight: 800; z-index: 100; opacity: 0; transition: 0.4s; box-shadow: 0 10px 30px rgba(var(--accent-rgb), 0.3);}
          ...
          <div className="glass-nav-wrapper fade-up-element">
            <div className="glass-nav">
              <div className="nav-left">
                ...
                <div className="glass-nav-active">🆚 最強シミュレーター</div>
              </div>
          ...
          <div className="sim-tabs-wrapper fade-up-element">
            <div className="sim-tabs">
              ...
              <button className={`sim-tab-btn ${activeTab === "air-talk" ? "active" : ""}`} onClick={() => setActiveTab("air-talk")}>📡 Air提案</button>
            </div>
          </div>
          ...
          {activeTab === "sb-hikari" && (
            <div className="main-layout">
              ...
              <section className="glass-panel fade-up-element" style={{ transitionDelay: "0.1s" }}>
                ...
                <div className="script-bubble" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>つまり、<span className="script-highlight">『実質半年間はタダで使って、一番いいタイミングで携帯キャリアに合わせて割安料金帯のものに乗り換えることもできる』</span>という、お客様の負担が一番少なく、かつお得になる『今人気のトレンドの使い方』ができます！</div>
              </section>
            </div>
          )}
            ...
            <div className="main-layout">
              <section className="glass-panel highlight-panel fade-up-element">
                ...
                <div className="result-grid">
                  ...
                  <div className="result-box total-box"><div className="result-label" style={{color:"rgb(var(--accent-rgb))", fontSize:"15px", fontWeight:900}}>💰 お客様へ渡す合計CB</div><div className="result-value" style={{color:"rgb(var(--accent-rgb))", fontSize:"26px"}}>¥{finalDocomoCbTotal.toLocaleString()}</div></div>
                </div>
                ...
                <div className="vs-container">
                  ...
                  <div className="vs-card good">
                    <span className="vs-badge" style={{background:"rgb(var(--accent-rgb))"}}>得</span>
              ...
              <section className="glass-panel fade-up-element" style={{ transitionDelay: "0.1s" }}>
                ...
                <div className="script-bubble script-me" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>
            ...
            <div className="main-layout">
              <section className="glass-panel highlight-panel fade-up-element">
                ...
                <div className="result-grid" style={{marginTop:"10px"}}>
                  ...
                  <div className="result-box total-box">
                    <div className="result-label" style={{color:"rgb(var(--accent-rgb))", fontSize:"16px", fontWeight:900}}>💰 提示する補填額 (CB Total)</div>
                    <div className="result-value" style={{color:"rgb(var(--accent-rgb))", fontSize:"28px"}}>¥{fletsCbTotal.toLocaleString()}</div>
                  </div>
                </div>
              </section>
              ...
              <section className="glass-panel fade-up-element" style={{ transitionDelay: "0.1s" }}>
                ...
                {fletsTalkScenario === 1 && (
                  <div className="fade-up-element visible">
                    ...
                    <div className="script-bubble" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>つまり、 『実質半年間はタダで使って、一番いいタイミングで携帯キャリアに合わせて乗り換えることもできる』 という、<span className="script-highlight">お客様の負担が一番少なく、かつお得になる『今人気のトレンドの使い方』</span>ができます！</div>
                ...
                {fletsTalkScenario === 3 && (
                  <div className="fade-up-element visible">
                    ...
                    <div className="script-bubble">そうすれば、半年後の切り替え時は、工事不要で簡単な手続きだけで済みますし、 その際の残債なども、乗り換え先のキャンペーン適応されて全額補填されます。</div>
                    <div className="script-bubble" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>つまり、 『実質半年間はタダで使って、一番いいタイミングで携帯キャリアに合わせて乗り換える』 という、<span className="script-highlight">お客様の負担が一番少なく、かつお得になる『今人気のトレンドの使い方』</span>ができます！</div>
                ...
                {fletsTalkScenario === 4 && (
                  <div className="fade-up-element visible">
                    ...
                    <div className="script-bubble" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>こちらご内容で問題なければフレッツ光で１度お試しいただければと思いますがご迷惑なかったでしょうか？？？</div>
                  </div>
                )}
            ...
            <div className="main-layout">
              <section className="glass-panel highlight-panel fade-up-element">
                ...
                <div className="result-grid">
                  ...
                  <div className="result-box total-box">
                    <div className="result-label" style={{color:"rgb(var(--accent-rgb))", fontSize:"15px", fontWeight:900}}>
                      💰 実質無料にするためのCB提示額
                    </div>
                    <div className="result-value" style={{color:"rgb(var(--accent-rgb))", fontSize:"28px"}}>
                ...
                <div style={{ marginTop: "20px" }}>
                  {airTalkType === "leo" && (
                    <div className="fade-up-element visible">
                       ...
                       <div className="script-bubble script-me" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>今回はレオパレスに合わせたプランとしてAirをご準備させていただいてます。</div>
                    </div>
                  )}
                  {airTalkType === "norack" && (
                    <div className="fade-up-element visible">
                       ...
                       <div className="script-bubble script-me" style={{background: "var(--card-hover-bg)", borderColor: "rgb(var(--accent-rgb))"}}>お客様の方で速度または料金どちらを求められますか？</div>
                    </div>
                  )}
                </div>
              </section>
