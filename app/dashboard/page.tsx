...
export default function SheetsDashboard() {
  ...
  // 💡 動的にAPIのURLを生成（選択中のシート情報を送る）
  const { data, error, isValidating } = useSWR(
    `/api/sheets-data?id=${activeSheet.id}&range=${activeSheet.range}`, 
    fetcher, 
  ...
  if (error) return <div className="p-10 text-red-500">通信エラー</div>;
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans p-4 md:p-8 relative">
      ...
      <div className="max-w-[1400px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-blue-600 p-6 rounded-3xl shadow-xl text-white md:col-span-2 flex justify-between items-center">
              <div>
                ...
                <p className="text-xl font-bold">{isValidating ? "📡 Syncing Latest 100..." : "✅ System Standby"}</p>
              </div>
              <button 
                onClick={scrollToBottom}
                className="bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/20 px-4 py-2 rounded-xl text-xs font-black transition-all"
              >
                ⬇️ 一番下へジャンプ
              </button>
            </div>
        ...
        <main className="bg-white border border-slate-200 rounded-[2.5rem] shadow-2xl overflow-hidden relative">
          <div 
            ...
            className="overflow-x-auto max-h-[60vh] overflow-y-auto scroll-smooth custom-scrollbar"
          >
                  ...
                  data.map((row: any, index: number) => (
                    <tr key={index} className={`group ${index % 2 !== 0 ? 'bg-slate-50/50' : 'bg-white'} hover:bg-blue-50/80 transition-colors`}>
                      ...
                      <td className="py-4 px-8">
                        <span className="px-2.5 py-1 bg-white border border-slate-200 rounded-md text-[10px] font-black text-slate-500 uppercase">{row.plan || "---"}</span>
                      </td>
                      {/* 💡 シンプル割列（データがある場合のみ表示されるロジック） */}
                      {activeSheet.cols.includes("シンプル割") && (
                        <td className="py-4 px-8 text-center">
                          {row.simpleWari === "確認しました" ? (
                            <span className="px-3 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded-full border border-emerald-200">確認しました</span>
                          ) : <span className="text-slate-300 text-[10px]">---</span>}
                        </td>
                      )}
                      <td className="py-4 px-8 text-center">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black ${row.jusetsu === "はい" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400"}`}>
                          {row.jusetsu || "---"}
                        </span>
                      </td>
                    </tr>
      ...
      {/* 🚀 フローティング・ジャンプボタン（右下に配置） */}
      <button 
        onClick={scrollToBottom}
        className="fixed bottom-10 right-10 w-14 h-14 bg-slate-900 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-50 group"
      >
